...
description: NDA(Non-Disclosure Agreement, 비밀유지계약서)를 검토하고 위험 요소를 분석합니다. 비밀정보 정의의 균형, 예외 조항(4대 요소), 비밀유지 기간, 책임상한, 준거법/관할, 파기확인서, 비차단선언 등 15개 핵심 검토 규칙을 적용하여 NDA 계약서를 체계적으로 검토합니다. 결과는 HIGH/MEDIUM/LOW 위험도별로 그룹핑하여 보고합니다. NDA 검토, 계약서 검토, 비밀유지계약서 분석, NDA clause review, NDA contract review가 필요할 때 이 스킬을 사용하세요.
---
...
다음 규칙들을 순서대로 검토합니다. 각 규칙에 대해 **위험도(HIGH/MEDIUM/LOW)**를 평가합니다. 검토 결과는 반드시 위험도별로 그룹핑해 요약합니다.
...
### 3. 검토 결과 요약 — 출력 형식(그룹핑 필수)

검토 완료 후 아래의 "위험도별 그룹 요약" 템플릿을 사용해 결과를 정리하세요. 결과는 반드시 HIGH → MEDIUM → LOW 순으로 그룹핑하여 제시합니다.

라벨링 및 정렬 규칙:
- 위험도 라벨은 반드시 대문자(HIGH, MEDIUM, LOW)를 사용합니다.
- 각 그룹 내에서는 비즈니스·법적 영향도가 큰 순서로 정렬합니다.
- 해당 사항이 없으면 "해당 없음"이라고 명시합니다.
...
## 계약 기본 정보
- 당사자: A회사 ↔ B회사 (회사 간/회사-개인)
- 유형: 상호 NDA/단일 NDA
- 목적: 기술협력/거래기본계약/채용 등

## 위험도별 그룹 요약
### HIGH 위험 (즉시 협상/수정 필요)
- [항목명 | 조문/섹션] 핵심 문제/근거 — 권고 수정안
- 예: [비밀정보 정의 | 제2조] 구두 공개 30일 확인 조항 부재 — 구두 공개 시 30일 내 서면 확인 조항 추가
- 예: [책임상한 | 제X조] 책임상한 부재 — 직접 손해 한도 및 금액 상한(예: 12개월치 수수료) 명시

### MEDIUM 위험 (협상 권장/대안 제시)
- [항목명 | 조문/섹션] 핵심 문제/근거 — 권고 수정안
- 예: [관련자 제공 | 제Y조] 사전 서면 동의 요구 — 동등 이상의 비밀유지 의무 조건으로 대체

### LOW 위험 (보완 권장/정보 정합성)
- [항목명 | 조문/섹션] 핵심 문제/근거 — 권고 수정안
- 예: [통지 조항 | 제Z조] 간주 수신 규정 부재 — 이메일 통지 5영업일 경과 간주수신 문구 삽입

## 제안 조치 요약
- 즉시 조치(High): 항목 요약 2~4개
- 협상 포인트(Medium): 항목 요약 2~4개
- 후속 보완(Low): 항목 요약 1~3개
```

선택 사항(요약 표가 필요한 경우에만 추가):
